...
const NORTE_TICKET_URL = 'https://norteticket.com.ar/eventos/salta'
...
/**
 * Scrapes events from NorteTicket website
 */
export async function scrapeNorteTicket(): Promise<ScrapedEvent[]> {
  ...
  try {
    ...
    // NorteTicket event card selectors (adjust based on actual DOM structure)
    // Common patterns: .event-card, .card-evento, .evento-item, article.event
    const cardSelectors = [
      '.event-card',
      '.card-evento', 
      '.evento-item',
    ...
    for (const selector of cardSelectors) {
      ...
      if (found.length > 0) {
        $cards = found
        console.log(`[v0] NorteTicket: Found ${found.length} events with selector "${selector}"`)
        break
      }
    }
    ...
    if ($cards.length === 0) {
      ...
      $cards = $('a[href*="/evento/"], a[href*="/event/"]').closest('div, article, li')
      console.log(`[v0] NorteTicket: Fallback found ${$cards.length} event containers`)
    }
    ...
    $cards.each((_, element) => {
      try {
        ...
        const event: ScrapedEvent = {
          ...
          flyerUrl: absoluteUrl(flyerUrl, NORTE_TICKET_URL),
          ticketLink: absoluteUrl(ticketLink, NORTE_TICKET_URL),
          source: 'norteticket'
        }
        
        events.push(event)
      } catch (cardError) {
        console.error('[v0] NorteTicket: Error parsing card:', cardError)
      }
    })
    
    console.log(`[v0] NorteTicket: Successfully scraped ${events.length} events`)
    
  } catch (error) {
    console.error('[v0] NorteTicket scraper error:', error)
  }
